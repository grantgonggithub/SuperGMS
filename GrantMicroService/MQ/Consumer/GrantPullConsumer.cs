...
namespace GrantMicroService.MQ.Consumer
{
    /// <summary>
    /// 拉模式消费者
    /// </summary>
    /// <typeparam name="M">消息内容</typeparam>
    public class GrantPullConsumer<M> : IDisposable
        where M : class
    {
        ...
        public GrantPullConsumer(MQueue queue)
        {
        ...
        public GrantPullConsumer(string exchange, string routeKey, string queueName, bool autoDelete, VirtualHost host)
            : this(new MQueue()
            {
                AutoDeclare = false,

...
namespace GrantMicroService.ApiHelper
{
    ...
    public class ApiHelper
    {
        ...
        private ClassInfo GetInterfaceInfo(Type type, Type argType, string lang = "zh_cn")
        {
            ...
            var processXmlNode = $"M:{type.FullName}.Process({argType.FullName},GrantMicroService.Protocol.RpcProtocol.StatusCode@)";
        ...
        private List<string> GetAllXml()
        {
            ...
            var di = fi.Directory;
            var xmlAlls = di.GetFiles("*.xml")?.Where(x => x.Name.StartsWith("grant.") || x.Name.StartsWith("GrantMicroService"));
            if (xmlAlls?.Count() > 0)
            {

...
namespace SuperGMS.Rpc
{
    ...
    public abstract class SuperGMSBaseServer : ISuperGMSRpcServer
    {
        ...
        public void RpcServerRegister(SuperGMSServerConfig server, string[] args)
        {
            try
            {
                ...
                if (server.ServerType == ServerType.Thrift || server.ServerType == ServerType.Grpc)
                {
                    ...
                    logger.LogWarning($"    rpc端口监听异常退出：{server.ServerName}{server.Port}{server.AssemblyPath}" + "   |  time=" +
                        DateTime.UtcNow.ToString("yy-MM-dd HH:mm:ss:ffff"));

                    if (ServerSetting.ConfigCenter.ConfigType == ConfigType.Zookeeper)
                    {
                        ZookeeperManager.ClearRouter(server.ServerName, server.Ip,server.Port);
                    }
            ...
            {
                logger.LogCritical(ex, "  Error 服务异常退出  " + DateTime.UtcNow.ToString("yy-MM-dd HH:mm:ss:ffff"));

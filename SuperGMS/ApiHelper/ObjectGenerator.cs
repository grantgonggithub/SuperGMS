...
namespace SuperGMS.ApiHelper
{
    ...
    public class ObjectGenerator
    {
        ...
        /// Gets or sets xml注释
        /// </summary>
        private  XmlCommentsFileCollection XmlCommencts { get; set; }

        /// <summary>
        /// Initializes a new instance of the <see cref="ObjectGenerator"/> class.
        /// </summary>
        public ObjectGenerator(XmlCommentsFileCollection xmls )
        {
            this.NoStringProperties = new List<KeyValuePair<string, string>>();
            this.XmlCommencts = xmls;
        }
        ...
        /// </summary>
        public List<KeyValuePair<string, string>> NoStringProperties { get; private set; }
        ...
        private object GenerateTuple(Type type, Dictionary<Type, object> createdObjectReferences)
        {
            ...
            ObjectGenerator objectGenerator = new ObjectGenerator(this.XmlCommencts);
        ...
        private object GenerateKeyValuePair(Type keyValuePairType, Dictionary<Type, object> createdObjectReferences)
        {
            ...
            NoStringProperties.AddRange(objectGenerator.NoStringProperties);
        ...
        private string GetDescription(Type type, PropertyInfo property)
        {
            ...
            ////string fieldName = C.R(property.Name);
            string pName = propertyType.Name;

            /*
            //var attRrequired = ReflectionTool.GetCustomAttributeEx<RequiredAttribute>(property);
            //if (attRrequired != null)
            //{
            //    descr.Add("*" + C.R("必输字段"));
            //}

            ////if (!string.IsNullOrEmpty(property.Name) && string.Compare(fieldName, pName, true) != 0)
            ////{
            ////    descr.Add(string.Format("【{0}】", fieldName));
            ////}
            ...
            //var length = AttrTool.GetStringLengthFromAttribute(null, property, type);
            //if (length > 0)
            //{
            //    descr.Add(C.R("最大长度") + length);
            //}

            ////var attrConvert = ReflectionTool.GetCustomAttributeEx<ConvertValueAttribute>(property);
            ////if (attrConvert != null)
            ////{
            ////    var dic = attrConvert.GetKeyNameDictionary();
            ////    if (dic.Count > 0)
            ////    {
            ////        descr.Add(C.R("值必须为") + string.Format("{0}等", string.Join(",", dic.Values.Take<string>(5))));
            ////    }
            ////}
            */
            ...
            if (objs.Length > 0)
            {
                descr.Add(((DescriptionAttribute)objs[0]).Description);
            }
            else
            {
            ...
            var str = GetXmlComment(type, property);
            if (!string.IsNullOrEmpty(str))
            {
                str = str.Replace("<summary>", string.Empty).Replace("</summary>", string.Empty)
                    .Replace("\r\n", string.Empty);
                str = str.Trim().Replace("           ", "<br />        ");
                descr.Add(str);
            }

            return string.Join(",", descr.ToArray());
        }
        ...
        private void SetPublicProperties(Type type, object obj, Dictionary<Type, object> createdObjectReferences)
        {
            ...
            foreach (PropertyInfo property in properties)
            {
                if (property.CanWrite)
                {
                    ...
                    NoStringProperties.AddRange(objectGenerator.NoStringProperties);
                    var desc = GetDescription(type, property);
                    if (!string.IsNullOrEmpty(desc))
                    {
                        NoStringProperties.Add(new KeyValuePair<string, string>(property.Name, desc));
                    }

                    /*
                    //if (property.PropertyType != null && property.PropertyType.Name == "String")
                    //{
                    //    if (!string.IsNullOrEmpty(desc))
                    //    {
                    //        property.SetValue(obj, desc, null);
                    //    }
                    //}
                    //else
                    //{
